...
#### 날씨 데이터 조회
- **요청 URL**: GET /api/weather
- **설명**: 지역명과 날씨 유형(POP, REH, TMP 등)을 기반으로 날씨 데이터를 조회합니다.
- **파라미터**:
  - `type` (String, 기본값: "POP"): 날씨 유형 (POP, REH, TMP, UUU 등)
  - `location` (String, 기본값: "서울"): 지역명
- **응답**:
  - 200: 조회 성공
  - 404: 지역명을 찾을 수 없음
  - 500: 서버 오류 또는 API 연결 오류

#### 모든 위치 정보 조회
- **요청 URL**: GET /api/weather/locations
- **설명**: 시스템에서 제공하는 모든 위치 정보를 페이징하여 조회합니다.
- **파라미터**:
  - `pageNo` (Integer): 페이지 번호
  - `numOfRows` (Integer): 페이지당 항목 수
- **응답**:
  - 200: 조회 성공 (페이지네이션 포함)
  - 500: 서버 오류
